 ...
 * ```
 */
export function withMaiatAcp(client, opts = {}) {
    const { minProviderScore = 60, autoEvaluator = true, evaluatorAddress = MAIAT_EVALUATOR_WALLET, apiKey, mode = 'block', onProviderWarn, onBeforeAccept, tokenGuard = true, } = opts;
    if (mode === 'silent')
        return client;
    ...
    return new Proxy(client, {
        get(target, prop, receiver) {
            ...
            if (prop === 'createJob' && typeof original === 'function') {
                return async function wrappedCreateJob(params) {
                    ...
                    const providerAddr = (params.providerAddress ?? params.provider);
                    if (providerAddr) {
                        ...
                        if (result) {
                            ...
                            if (isLowTrust) {
                                if (mode === 'block') {
                                    throw new MaiatAcpError(`Job creation blocked: provider ${providerAddr} has trust score ${result.score}/100`, providerAddr, result);
                                }
                                if (mode === 'warn') {
                        ...
                        for (const key of ['token', 'tokenAddress', 'tokenOut', 'tokenIn']) {
                            ...
                            if (typeof addr === 'string' && /^0x[0-9a-fA-F]{40}$/.test(addr)) {
                                ...
                                if (tokenResult && (tokenResult.verdict === 'danger' || tokenResult.isHoneypot)) {
                                    if (mode === 'block') {
                                        throw new MaiatAcpError(`Job creation blocked: token ${addr} is dangerous — ${tokenResult.riskSummary}`, addr, null);
                                    }
                                }
                            }
                        }
            ...
            if (prop === 'acceptJob' && typeof original === 'function') {
                return async function wrappedAcceptJob(jobId, ...args) {
                    ...
                    if (providerAddr) {
                        ...
                        if (onBeforeAccept) {
                            ...
                            if (!allowed) {
                                throw new MaiatAcpError(`Job ${jobId} rejected by onBeforeAccept hook`, providerAddr, result);
                            }
                        }
                        if (result) {
                            const isLowTrust = result.verdict === 'block' || result.score < minProviderScore;
                            if (isLowTrust) {
                                if (mode === 'block') {
                                    throw new MaiatAcpError(`Job ${jobId} rejected: provider ${providerAddr} trust score ${result.score}/100`, providerAddr, result);
                                }
                                if (mode === 'warn') {
                                    onProviderWarn?.(providerAddr, result);
                                }
                    ...
                    return original.call(target, jobId, ...args);
                };
            }
            return original;
        },
...
export class MaiatAcpError extends Error {
    constructor(message, address, trustResult) {
        super(message);
...
//# sourceMappingURL=acp-wrap.js.map
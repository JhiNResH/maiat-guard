...
export class MaiatTokenError extends Error {
    constructor(result) {
        super(`Swap blocked: token ${result.address} is ${result.verdict} — ${result.riskSummary}`);
 ...
 */
export function isSwapTransaction(to) {
    if (!to)
        return false;
 ...
 */
export function addRouter(address) {
 ...
 */
export function extractTokenOut(data) {
 ...
 */
function extractFromUniversalRouter(data) {
 ...
 */
function extractLastFromPath(data) {
    const hexBody = data.slice(10);
 ...
 */
export async function checkToken(tokenAddress, apiKey) {
    ...
    try {
        const headers = { 'X-Maiat-Client': 'maiat-guard' };
        if (apiKey)
            headers['X-Maiat-Key'] = apiKey;
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), TIMEOUT_MS);
        const res = await fetch(`${MAIAT_API}/api/v1/token/${lower}`, { headers, signal: controller.signal }).finally(() => clearTimeout(timer));
        if (res.status === 404)
            return null;
        if (!res.ok)
            return null;

 ...
 */
export function detectVanityMatch(target, knownAddresses) {
    ...
    for (const known of knownAddresses) {
        ...
        if (k.length !== 40)
            continue;
        if (k === t)
            continue; // same address, not a match
 ...
 */
async function fetchRecentCounterparties(walletAddress, config) {
 ...
 */
async function checkLiveness(address, config) {
    const explorerUrl = config.explorerUrl ?? DEFAULT_EXPLORER;
    const apiKey = config.etherscanApiKey ?? '';
    try {
        const controller = new AbortController();
 ...
 */
export async function antiPoisonGate(targetAddress, walletAddress, config) {
...
//# sourceMappingURL=anti-poison.js.map